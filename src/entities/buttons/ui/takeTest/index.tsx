...
export const TakeTestButton = ({ children, onClick }: ButtonProps) => {

  return (
    <div className={style.wrapper} >
      <div className={cn(style.button, fonts.buttonTextNorm)} onClick={() => { onClick && onClick() }}>{children}</div>
      <div className={fonts.signText}>Не уверены, какую<br /><a href={"#"} className={fonts.linkTextMini}>профессию</a> выбрать?</div>
    </div>
  )
};
